...
needed_binaries=(lsb_release wget add-apt-repository gpg update-alternatives)
missing_binaries=()
...
PKG="clang-$LLVM_VERSION clangd-$LLVM_VERSION"
if [[ $ALL -eq 1 ]]; then
    PKG="$PKG clang-tidy-$LLVM_VERSION clang-format-$LLVM_VERSION clang-tools-$LLVM_VERSION libllvm$LLVM_VERSION llvm-$LLVM_VERSION-dev llvm-$LLVM_VERSION-tools libclang-common-$LLVM_VERSION-dev libclang-$LLVM_VERSION-dev libclang-cpp$LLVM_VERSION-dev"
    PKG="$PKG libmlir-$LLVM_VERSION-dev mlir-$LLVM_VERSION-tools"
